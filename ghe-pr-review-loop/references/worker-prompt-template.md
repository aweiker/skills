...
19. After posting EVERY reply (actionable or non-actionable), also check the appropriate feedback checkbox on the original bot comment. Use the quality-to-checkbox mapping from the "Check Feedback Checkbox on Bot Comments" section in worker-mode.md. This is mandatory for every reply — it provides structured quality signal back to the bot system.
20. Never write "deferred", "follow-up", or "will address later" in any reply, HANDOFF, or comment without a concrete issue URL created in the same action via `gh issue create`. Every actionable finding that is not fixed must have a created tracking issue. If the finding is not worth creating an issue for, it is not worth mentioning as deferred — classify it as out-of-scope and move on.
```
