...
generate_impl_prompt() {
  ...
  if [ "${AI_REVIEW_PROVIDER:-}" = "coderabbit" ] && [ "${LOCAL_CODERABBIT_PRECHECK:-1}" = "1" ]; then
    ...
    echo "   - Agreement and disagreement both require evidence: accepted findings name the violated invariant and proof; rejected findings cite exact code/test/contract/scope evidence."
    echo "   - For false-positive, stale, or explicitly out-of-scope findings, run coderabbit feedback with the evidence (for example: coderabbit feedback \"False positive: <finding>. Evidence: <file/test/contract>. No change made because <reason>.\"), record the rationale in the PR body and handoff, and do not churn low-value suggestions."
